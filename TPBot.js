...
loadPlayerPearlUuid()
//load credentials.txt from file credentials.txt are ip, username, password, chatEncryptionKey
...
const connect = () => {
        ...
        instance.on('whisper', (username, message) => {
            ...
            console.log("Dif: " + (getNanoSecTime() - lastPlayerCommand.get(username)))
            lastPlayerCommand.set(username, getNanoSecTime())
        ...
        instance.on('blockUpdate', (oldBlock, newBlock) => {
            ...
            // all players in range of 20 blocks
            let playersInRange = ""
            players.forEach(player => {
                if (player.position.distanceTo(newBlock.position) < 20) {
